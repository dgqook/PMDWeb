...
<input type="hidden" name="servletMessage" value= "${servletMessage}"/>
	...
	<div id="mid">
		<div id="mid_wrap">
			...
			<script type="text/javascript">
				function extend(form){
				  var f= form;
				  if(confirm("사용 기간을 연장하시겠습니까?")) {
					  f.method="post";
					  f.action="${pageContext.request.contextPath}/web/work/extendPage.do";
					  f.submit();
				  }
				}
				...
				google.charts.load('current', {'packages':['table']});
			      ...
			      function drawTable() {
			        ...
			        data.addColumn('string', '휴대전화');
			        data.addColumn('string', '최근접속일');
			        ...
			        data.addRows([
						...
						['<%=userList.get(i).getUserId()%>',
						 ...
						 '<%=userList.get(i).getUserCoName()%>',
						 '<%=userList.get(i).getUserTel()%>',
						 '<%=userList.get(i).getUserHp()%>',
						 '<%=userList.get(i).getUserRecentConn()%>',
						 '<%=userList.get(i).getUserExpiryDate()%>',
						 '<form>'+ 
						 	<%if(userList.size()>=1&&!userList.get(i).getUserId().equals("")){%>
					 	 	'<input type="button" value="기간연장" onclick="extend(this.form)">'+
					 	 	'<input type="hidden" name="userId" value="<%=userList.get(i).getUserId()%>">'+
					 	 	'<input type="hidden" name="userExpiryDate" value="<%=userList.get(i).getUserRegDate()%>">'+
					 	 	<%}%>
				 	 	'</form>']
