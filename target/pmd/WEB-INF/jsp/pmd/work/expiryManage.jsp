...
<%@ page import="pmd.common.vo.ExpiryManageVO" %>
...
<%	
	...
	ArrayList<ExpiryManageVO> expiryList= (ArrayList<ExpiryManageVO>)session.getAttribute("expiryList");
	if(expiryList == null){
		expiryList= new ArrayList<ExpiryManageVO>();
	}
%>
<!DOCTYPE html>
			...
			<script type="text/javascript">
				google.charts.load('current', {'packages':['table']});
			      ...
			      function drawTable() {
			        ...
			        data.addColumn('string', '유선전화');
			        data.addColumn('string', '무선전화');
			        data.addColumn('string', '소프트웨어명');
			        data.addColumn('string', '수량');
			        data.addColumn('string', '잔여기간');
			        
			        data.addRows([
						<%if(expiryList!=null && expiryList.size()!=0){
							for(int i=0; i<expiryList.size(); i++){%>
						['<%=expiryList.get(i).getUserId()%>','<%=expiryList.get(i).getUserCoName()%>',
						 '<%=expiryList.get(i).getUserName()%>','<%=expiryList.get(i).getUserTel()%>',
						 '<%=expiryList.get(i).getUserHp()%>','<%=expiryList.get(i).getSwName()%>',
						 '<%=expiryList.get(i).getOwnQuantity()%>','<%=expiryList.get(i).getOwnExpDate()%>'
						 ]<%if(i!=(expiryList.size()-1)){%>,<%}}}%>
			        ]);
